(function () {
  ...
  const DRAFT_KEY = 'velocity-pelaez-spata-draft-v1';
  ...
  // Known facts only. Addresses, phones, and hours come from each studio's
  // official stretchzone.com location page (verified 2026-07-31 against the
  // page's schema.org openingHours block and cross-checked against Yelp).
  // Studio emails appear on the CRM leads for both studios, independently of
  // the location pages (whose published emails are not trusted — Brickell's
  // page carried a dummy). ClubReady store IDs supplied by George 2026-07-31.
  //
  // `city` is the postal city; West Bradenton's card headline carries the name
  // the studio actually uses, and `label` is the short form for messages.
  const LOCATIONS = [
    {
      key: 'west-bradenton', name: 'Stretch Zone West Bradenton', city: 'Bradenton', label: 'West Bradenton',
      address: '5917 Manatee Ave West, Suite 611, West Bradenton, FL 34209',
      phone: '(941) 281-2673',
      email: 'sz.west.bradenton.fl@stretchzone.com',
      storeId: '11056',
      website: 'https://www.stretchzone.com/locations/west-bradenton-fl',
      gbp: 'https://www.google.com/maps/search/?api=1&query=Stretch%20Zone%205917%20Manatee%20Ave%20West%20Suite%20611%20Bradenton%20FL%2034209',
      hours: { mon: ['08:00', '20:00'], tue: ['08:00', '20:00'], wed: ['08:00', '20:00'], thu: ['08:00', '20:00'], fri: ['08:00', '20:00'], sat: ['08:00', '16:00'], sun: ['08:00', '16:00'] }
    },
    {
      key: 'sarasota-south', name: 'Stretch Zone Sarasota South', city: 'Sarasota', label: 'Sarasota South',
      address: '2881 Clark Road, Suite 13, Sarasota, FL 34231',
      phone: '(941) 552-8029',
      email: 'sz.sarasota.south.fl@stretchzone.com',
      storeId: '13118',
      website: 'https://www.stretchzone.com/locations/sarasota-south-fl',
      gbp: 'https://www.google.com/maps/search/?api=1&query=Stretch%20Zone%202881%20Clark%20Road%20Suite%2013%20Sarasota%20FL%2034231',
      hours: { mon: ['07:00', '19:00'], tue: ['07:00', '19:00'], wed: ['07:00', '19:00'], thu: ['07:00', '19:00'], fri: ['07:00', '19:00'], sat: ['08:00', '14:00'], sun: ['08:00', '14:00'] }
    }
  ];
  ...
  function locationCardHTML(loc, i) {
    ...
    <section class="mg-loc-card" data-loc="${i}">
      ...
      <label>
        Studio Email ${knownChip('From our records')}
        <input type="email" name="loc${i}_business_email" value="${loc.email}" data-known>
        <small>The inbox this studio uses with leads and members.</small>
      </label>
      <label>
        ClubReady Store ID ${knownChip('From our records')}
        <input type="text" name="loc${i}_crm_store_id" value="${loc.storeId}" data-known>
      </label>
      <label>
  ...
  function accessOptionsHTML(selected) {
    const opts = [['all', 'Both studios']]
      .concat(LOCATIONS.map((loc, i) => [`loc${i}`, `${loc.label} only`]));
  ...
  function renderAll() {
    ...
    // Names and emails supplied by George 2026-07-31. Macarena Pelaez Alcala
    // ("Maqui") is the owner on the official stretchzone.com West Bradenton
    // page and our CRM records; Aferdita Spata goes by "Dede" (spelling of the
    // full name taken from the email address George provided — easy for her to
    // correct on the form if we got it wrong).
    addUser('Macarena Pelaez Alcala', 'mariamacarenapelaezalcala@gmail.com', 'all');
    addUser('Aferdita Spata', 'aferdita.spata@gmail.com', 'all');
  ...
  // The raw field map. Used for the localStorage draft AND stored on the first
  // studio's server row as form_state, so a draft rehydrates exactly. The
  // mapped columns cannot do that: buildPayloads composes `notes` lossily.
  ...
  // tailored form doesn't ask), so both rows flow through the same
  ...
  // submit. firstId lets draft-sync keep one stable row for studio 1, which is
  // the row that carries the draft for the whole form.
  function buildPayloads(status, firstId) {
    const fd = new FormData(document.getElementById('intake-form'));
    ...
    const shared = {
      ...
      // George confirmed 2026-07-31 that both studios are on ClubReady.
    ...
    return LOCATIONS.map((loc, i) => {
      ...
      const noteParts = [
        `Pelaez/Spata multi-location intake (${i + 1} of ${LOCATIONS.length}). Primary contact: ${contactName || 'Aferdita Spata'}.`,
        'Pricing, intro offer, and cancellation copy were pre-filled from other Stretch Zone locations as examples — confirm against this studio before provisioning.',
      ...
      return Object.assign({}, shared, {
        id: i === 0 ? (firstId || crypto.randomUUID()) : crypto.randomUUID(),
        // Only studio 1 carries the raw form state: it is the draft row, and
        // one authoritative copy beats two that could drift.
        form_state: i === 0 ? collectFormState() : null,
  ...
  function setSyncLabel(state) {
    const el = document.querySelector('.mg-autosave-note');
    if (!el) return;
  ...
  const sync = VelocityDraftSync.create({
    ...
    preview: PREVIEW,
    // Studio 1's row is the draft carrier for the whole form.
    buildPayload: (status, id) => buildPayloads(status, id)[0],
    // Studio 1 has just been flipped to pending by draft-sync; insert the rest.
    afterSubmit: async (firstId) => {
      const rest = buildPayloads('pending', firstId).slice(1);
      for (const p of rest) await insertRow(p);
    },
  ...
  function showAlreadySubmitted() {
    ...
    s.querySelector('p').textContent = 'We have your onboarding details for West Bradenton and Sarasota South. If something needs changing, email bill@velocityaipartners.ai and we will update it for you.';
    s.hidden = false;
  }
  ...
  function showSuccess() {
    document.getElementById('intake-form').hidden = true;
    const s = document.getElementById('success-screen');
    if (PREVIEW) s.querySelector('p').textContent = 'Preview mode: nothing was actually sent. On the live link this submits both studios to the Velocity team.';
    s.hidden = false;
  ...
  async function doFinalSubmit() {
    ...
    try {
      ...
      // Flips studio 1's draft row to 'pending' (or inserts it outright), then
      // afterSubmit inserts studio 2.
      const intakeId = await sync.submit();
      // One receipt email, not two — the n8n workflow re-reads the row and
      ...
      btn.disabled = false;
      btn.textContent = 'Confirm & submit both studios';
    }
  }
