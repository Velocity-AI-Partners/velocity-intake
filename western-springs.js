(function () {
  ...
  const DRAFT_KEY = 'velocity-western-springs-draft-v1';
  ...
  // Single studio: Western Springs, IL. Owners on record are Michael and
  // Wesley (first names only — George supplied them; no surname, personal
  // email, or phone exists anywhere verifiable, so the contact section stays
  // blank for them to fill).
  //
  // Known facts only: address, phone, and hours from the official
  // stretchzone.com/locations/western-springs-il page (fetched 2026-07-31).
  // Studio email matches the standard sz.<city>.<st>@stretchzone.com pattern
  // and appears on both CRM leads for this studio. The store ID comes from the
  // Feb 2026 SZ scorecard's location line "Western Springs, IL (11995)" — same
  // parenthetical format that held ClubReady store IDs for West Bradenton and
  // Sarasota South — pre-filled with a verify-me chip, not asserted as fact.
  //
  // A day with `null` hours renders pre-checked "closed" (Sunday here — first
  // studio in the series that isn't open 7 days).
  const LOCATIONS = [
    {
      key: 'western-springs', name: 'Stretch Zone Western Springs', city: 'Western Springs', label: 'Western Springs',
      address: '4700 Gilbert Avenue, Unit 43B, Western Springs, IL 60558',
      phone: '(708) 469-7388',
      email: 'sz.western.springs.il@stretchzone.com',
      storeId: '11995',
      website: 'https://www.stretchzone.com/locations/western-springs-il',
      gbp: 'https://www.google.com/maps/search/?api=1&query=Stretch%20Zone%204700%20Gilbert%20Avenue%20Unit%2043B%20Western%20Springs%20IL%2060558',
  ...
  function locationCardHTML(loc, i) {
    ...
    <section class="sk-loc-card" data-loc="${i}">
      ...
      <label>
        ClubReady Store ID ${knownChip('From SZ scorecard — please verify')}
  ...
  function buildPayload(status, id) {
    ...
    const noteParts = [
      `Western Springs (IL) Stretch Zone intake. Owners on record: Michael and Wesley (first names only — no verified surnames or personal contacts; the contact fields are client-supplied). Primary contact: ${contactName || 'not given'}.`,
      'Store ID 11995 was pre-filled from the Feb 2026 SZ scorecard — verify before provisioning. Pricing, intro offer, and cancellation copy were pre-filled from other Stretch Zone locations as examples — confirm against Western Springs.',
    ...
    return {
      ...
      // Western Springs is a Chicago suburb — Central time, unlike every
      // earlier variant in this series.
      ...
      // ClubReady is the Stretch Zone brand standard and the scorecard store
      // number matches ClubReady's ID format, but this studio's platform has
  ...
  function showAlreadySubmitted() {
    ...
    s.querySelector('p').textContent = 'We have your onboarding details for Western Springs. If something needs changing, email bill@velocityaipartners.ai and we will update it for you.';
    s.hidden = false;
  }
  ...
  function showSuccess() {
    document.getElementById('intake-form').hidden = true;
    const s = document.getElementById('success-screen');
    if (PREVIEW) s.querySelector('p').textContent = 'Preview mode: nothing was actually sent. On the live link this submits the Western Springs studio to the Velocity team.';
    s.hidden = false;
