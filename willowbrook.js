(function () {
  ...
  const DRAFT_KEY = 'velocity-willowbrook-draft-v1';
  ...
  // Single studio: Willowbrook, IL — the second form for Michael and Wesley
  // (first names only; same owners as the Western Springs form, which has its
  // own page and draft row). No surname, personal email, or phone exists
  // anywhere verifiable, so the contact section stays blank for them to fill.
  //
  // Known facts only: address, phone, and hours from the official
  // stretchzone.com/locations/willowbrook-il page (fetched 2026-07-31).
  // Studio email matches the standard sz.<city>.<st>@stretchzone.com pattern
  // and appears on both CRM leads for this studio. Store ID 10865 was supplied
  // by George directly ("Willowbrook, IL (10865)") — same parenthetical format
  // that held ClubReady store IDs for West Bradenton and Sarasota South —
  // pre-filled with a verify-me chip. The page's tracking code carries an
  // unlabeled "18109"; George's number wins.
  //
  // A day with `null` hours renders pre-checked "closed" (Sunday here, same
  // as Western Springs).
  const LOCATIONS = [
    {
      key: 'willowbrook', name: 'Stretch Zone Willowbrook', city: 'Willowbrook', label: 'Willowbrook',
      address: '876 75th Street, Willowbrook, IL 60527',
      phone: '(630) 323-9983',
      email: 'sz.willowbrook.il@stretchzone.com',
      storeId: '10865',
      website: 'https://www.stretchzone.com/locations/willowbrook-il',
      gbp: 'https://www.google.com/maps/search/?api=1&query=Stretch%20Zone%20876%2075th%20Street%20Willowbrook%20IL%2060527',
      hours: { mon: ['07:00', '20:00'], tue: ['07:00', '20:00'], wed: ['07:00', '20:00'], thu: ['07:00', '20:00'], fri: ['07:00', '20:00'], sat: ['08:00', '16:00'], sun: null }
    }
  ];
  ...
  function hoursGridHTML(i, loc) {
    return DAYS.map(d => {
      ...
      <input type="time" name="loc${i}_hours_${d}_open" value="${closed ? '' : h[0]}"${closed ? ' disabled' : ''}>
      <input type="time" name="loc${i}_hours_${d}_close" value="${closed ? '' : h[1]}"${closed ? ' disabled' : ''}>
      <label class="closed-wrap"><input type="checkbox" name="loc${i}_hours_${d}_closed"${closed ? ' checked' : ''}> closed</label>
    `;
    }).join('');
  }
  ...
  function locationCardHTML(loc, i) {
    ...
    <section class="sk-loc-card" data-loc="${i}">
      ...
      <label>
        ...
        <small>The inbox this studio uses with leads and members. Correct it if this isn't right.</small>
      </label>
      <label>
        ClubReady Store ID ${knownChip('From SZ records — please verify')}
        <input type="text" name="loc${i}_crm_store_id" value="${loc.storeId}" data-known>
        <small>We believe this is your ClubReady store number. Please correct it if it's wrong.</small>
      </label>
      <label>
  ...
  function renderAll() {
    ...
    // Both owners are seeded by first name; emails are unknown, so the rows
    // are left for them to complete.
    addUser('Michael', '');
    addUser('Wesley', '');
  ...
  function applyFormState(data) {
    ...
    // Sunday seeds pre-checked "closed", and a saved draft only records
    // checkboxes that were ON — so a client who un-checked a closed day would
    // get it silently re-checked on restore. For hours checkboxes the saved
    // state is authoritative in both directions: absent means open.
    document.querySelectorAll('#intake-form [name$="_closed"]').forEach(el => {
      const want = Object.prototype.hasOwnProperty.call(data, el.name);
      if (el.checked !== want) {
        el.checked = want;
  ...
  function buildPayload(status, id) {
    ...
    const noteParts = [
      `Willowbrook (IL) Stretch Zone intake. Owners on record: Michael and Wesley (first names only — no verified surnames or personal contacts; the contact fields are client-supplied). Same owners as the Western Springs intake — review the two rows together. Primary contact: ${contactName || 'not given'}.`,
      'Store ID 10865 was supplied by George (2026-07-31) — verify before provisioning. Pricing, intro offer, and cancellation copy were pre-filled from other Stretch Zone locations as examples — confirm against Willowbrook.',
    ...
    return {
      ...
      // Willowbrook is a Chicago suburb — Central time, like its Western
      // Springs sibling and unlike the earlier Eastern-time variants.
      ...
      // ClubReady is the Stretch Zone brand standard and George supplied the
      // store number in ClubReady's ID format, but this studio's platform has
      // not been individually confirmed — the store-ID chip asks the client
      // to verify.
      ...
      // instead, so treat it as a starting point and confirm with the owners.
      automation_goals: {
  ...
  function showAlreadySubmitted() {
    ...
    s.querySelector('p').textContent = 'We have your onboarding details for Willowbrook. If something needs changing, email bill@velocityaipartners.ai and we will update it for you.';
    s.hidden = false;
  }
  ...
  function showSuccess() {
    document.getElementById('intake-form').hidden = true;
    const s = document.getElementById('success-screen');
    if (PREVIEW) s.querySelector('p').textContent = 'Preview mode: nothing was actually sent. On the live link this submits the Willowbrook studio to the Velocity team.';
    s.hidden = false;
